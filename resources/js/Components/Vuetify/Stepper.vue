<!-- resources/js/Components/Vuetify/Stepper.vue -->
<template>
    <v-stepper
    ...
    >
        ...
        >
            ...
            <v-stepper-header v-if="items && items.length">
                <template v-for="(step, index) in items" :key="String(step.value)">
                    <template v-if="$slots['header-item']">
                        <slot name="header-item" :item="step" :index="index"/>
                    </template>
                    <template v-else>
            ...
            <v-stepper-window v-if="items && items.length">
                <v-stepper-window-item v-for="step in items" :key="String(step.value)" :value="step.value">
                    <!-- slot item.{value} tiene prioridad -->
                    <slot  :name="`item.${step.value}`" :step="step">

                    <slot name="item" :step="step">
                        <div>
                            <h3 class="text-h6 text-uppercase">
                                {{ step.title }}
                            </h3>
                            <p v-if="step.subtitle" class="text-h6 text-uppercase">
                                {{ step.subtitle }}
                            </p>
                        </div>
                    </slot>
            ...
            <div v-if="!hideActions && items && items.length && $slots.actions" class="justify-center gap-4 px-4 pt-2 pb-4 space-x-4 d-flex align-center">
                <slot
                    name="actions"
                    :prev="prev"
                    :next="() => handleNext(next)"
